...
export interface ListRoster extends RosterAttributes {
  costs: CostAttributes[];
  forces: ListForce[];
  forceMap: ListForceMap;
}

export interface ListForceMap {
  [id: string]: ListForce;
}

export interface ListForce extends ForceAttributes {
  categories: CategoryAttributes[];
  publications: PublicationAttributes[];
  rules: ListRule[];
  selections: ListSelection[];
  map: ListForceSelectionMap;
}

export interface ListForceSelectionMap {
  [category: string]: ListSelection[];
}

export interface ListRule extends RuleAttributes {
  description: string[];
}

export interface ListSelection extends SelectionAttributes {
  costs: CostAttributes[];
  categories?: CategoryAttributes[];
...
export interface ListProfile extends ProfileAttributes {
  characteristics: ListCharacteristic[];
}

export interface ListCharacteristic extends CharacteristicAttributes {
  value: string;
}
